...
app.get('/', (req, res) => {
    const htmlLines = [
        ...
        '                        <input type="text" id="master-search" placeholder="Search Master Drugs..." oninput="filterMasterDrugs()">',
        '                        <div class="table-wrap" style="max-height: 480px;">',
        '                            <table>',
        '                                <thead><tr><th>Drug Name</th><th style="text-align:right;">Action</th></tr></thead>',
        '                                <tbody id="master-drugs-table"></tbody>',
        '                            </table>',
        '                        </div>',
        '                    </div>',

        '                    <!-- PANEL 2: REALTIME DISPENSE & QUANTITY ENGINE -->',
        '                    <div class="panel">',
        '                        <h2>⚡ Realtime Dispense Engine</h2>',
        '                        <div style="background: #f8fafc; padding: 12px; border-radius: 8px; border: 1px solid #d1d9e6; margin-bottom: 15px;">',
        '                            <div style="font-size: 11px; color: var(--text-muted); font-weight: bold; margin-bottom: 4px;">SELECTED DRUG:</div>',
        '                            <div id="selected-drug-display" style="font-size: 15px; font-weight: bold; color: var(--accent);">-- None Selected --</div>',
        '                        </div>',
        '                        <input type="number" id="dispense-qty" placeholder="Quantity to dispense" value="1" min="1">',
        '                        <div class="qty-grid">',
        '                            <div class="qty-pill" onclick="addQty(1)">+1</div>',
        '                            <div class="qty-pill" onclick="addQty(5)">+5</div>',
        '                            <div class="qty-pill" onclick="addQty(10)">+10</div>',
        '                            <div class="qty-pill" onclick="addQty(50)">+50</div>',
        '                        </div>',
        '                        <button onclick="submitDispense()" class="primary-btn success" style="padding: 12px; font-size: 14px; margin-top: 5px;">RECORD DISPENSE ENTRY</button>',
        '                        <br><br>',
        '                        <h2>📊 Live Cumulative Totals</h2>',
        '                        <div class="table-wrap" style="max-height: 300px;">',
        '                            <table>',
        '                                <thead><tr><th>Drug Name</th><th style="text-align:right;">Total Qty</th><th style="text-align:right;">Action</th></tr></thead>',
        '                                <tbody id="cumulative-table"></tbody>',
        '                            </table>',
        '                        </div>',
        '                    </div>',

        '                    <!-- PANEL 3: TRANSACTIONS & EXPORTS -->',
        '                    <div class="panel">',
        '                        <h2>📝 Live Transaction Ledger</h2>',
        '                        <div class="table-wrap" style="max-height: 250px; margin-bottom: 15px;">',
        '                            <table>',
        ...
        '                        <button onclick="exportExcel()" class="primary-btn success" style="margin-bottom: 8px;">Export to Excel (.xlsx)</button>',
        '                        <button onclick="exportPDF()" class="primary-btn danger" style="margin-bottom: 8px;">Export to PDF (.pdf)</button>',
        '                        <button onclick="clearZoneTotals()" class="primary-btn warning" style="margin-top: 10px;">Clear All Zone Totals</button>',
        '                    </div>',
        '                </div>',
        '            </div>',
        '        </div>',
        ...
        '        <div class="panel" style="width: 400px; margin: 0; background: white;" id="modal-content"></div>',
        '    </div>',
        ...
        '                if (data.error === "ACCESS_DISABLED" || data.message) {',
        '                    alert(data.message || "Access Disabled");',
        '                }',
        '                handleLogout();',
        '            }',
        '            return data;',
        ...
        '            errDiv.textContent = "";',
        '            if (!u || !p) { errDiv.textContent = "Please enter username and password."; return; }',
        ...
        '            if (authToken) {',
        '                await apiFetch("/api/logout", { method: "POST", body: { zone: activeZone } });',
        '            }',
        ...
        '        async function initAppSession(userData) {',
        ...
        '            await loadZones();',
        '            if (zonesRegistry.length > 0) {',
        '                activeZone = zonesRegistry[0];',
        '                setupZoneUI();',
        '                await refreshData();',
        '            }',

        '            if (syncInterval) clearInterval(syncInterval);',
        '            syncInterval = setInterval(refreshData, 2000);',
        '        }',

        '        async function loadZones() {',
        '            zonesRegistry = await apiFetch("/api/zones");',
        '            if (!Array.isArray(zonesRegistry)) zonesRegistry = [];',
        '        }',

        '        function setupZoneUI() {',
        '            const pickerWrap = document.getElementById("user-zone-picker-wrap");',
        '            const singleWrap = document.getElementById("single-zone-badge-wrap");',
        '            const selectEl = document.getElementById("user-zone-select");',

        '            selectEl.innerHTML = "";',
        '            zonesRegistry.forEach(z => {',
        ...
        '            if (zonesRegistry.length > 1 || currentUserRole === "ADMIN") {',
        '                pickerWrap.style.display = "block";',
        '                singleWrap.style.display = "none";',
        '            } else {',
        '                pickerWrap.style.display = "none";',
        '                singleWrap.style.display = "block";',
        '                document.getElementById("single-zone-name").textContent = activeZone || "NONE";',
        '            }',
        '        }',
        ...
        '            masterDrugsList = await apiFetch("/api/master-drugs?zone=" + encodeURIComponent(activeZone));',
        '            dispenseLogs = await apiFetch("/api/dispense/sync?zone=" + encodeURIComponent(activeZone));',
        '            renderMasterDrugs();',
        '            renderDispenses();',
        '            if (currentUserRole === "ADMIN") {',
        '                loadAdminActivityAndAudit();',
        '            }',
        '        }',

        '        async function loadAdminRegistry() {',
        '            const zones = await apiFetch("/api/zones");',
        '            const zContainer = document.getElementById("zone-checkbox-container");',
        '            zContainer.innerHTML = "";',
        '            zones.forEach(z => {',
        '                const lbl = document.createElement("label");',
        '                lbl.className = "zone-checkbox-item";',
        '                lbl.innerHTML = `<input type="checkbox" value="${z}"> ${z}`;',
        '                zContainer.appendChild(lbl);',
        '            });',

        '            const importSel = document.getElementById("admin-import-zone");',
        '            importSel.innerHTML = \'<option value="">-- Select Target Zone --</option>\';',
        '            zones.forEach(z => {',
        '                const opt = document.createElement("option");',
        '                opt.value = z;',
        '                opt.textContent = z;',
        ...
        '        async function loadAdminActivityAndAudit() {',
        '            const acts = await apiFetch("/api/admin/activity-logs");',
        '            const audits = await apiFetch("/api/admin/audit-logs");',
        '            ',
        '            const actBody = document.getElementById("activity-table");',
        '            actBody.innerHTML = "";',
        '            acts.forEach(a => {',
        '                actBody.innerHTML += `<tr><td>${a.timestamp}</td><td><b>${a.username}</b></td><td>${a.phone}</td><td><span class="badge">${a.action}</span></td></tr>`;',
        '            });',

        '            const audBody = document.getElementById("audit-table");',
        '            audBody.innerHTML = "";',
        '            audits.forEach(ad => {',
        '                audBody.innerHTML += `<tr><td>${ad.timestamp}</td><td><b>${ad.username}</b></td><td>${ad.phone}</td><td>${ad.zone}</td><td><span class="badge">${ad.action}</span></td></tr>`;',
        '            });',
        '        }',
        ...
        '            if (!newUsername || !newPassword) { alert("Please provide both username and password."); return; }',
        '            const res = await apiFetch("/api/admin/profile", { method: "PUT", body: { newUsername, newPassword } });',
        ...
        '            if (!zName) return;',
        '            const res = await apiFetch("/api/zones", { method: "POST", body: { zone_name: zName } });',
        '            alert(res.message || res.error);',
        ...
        '            const checkboxes = document.querySelectorAll("#zone-checkbox-container input:checked");',
        ...
        '            const res = await apiFetch("/api/users", { method: "POST", body: { username, phone, password, zones } });',
        '            alert(res.message || res.error);',
        '            if (!res.error) {',
        ...
        '            await apiFetch("/api/users/" + id + "/status", { method: "PUT", body: { status: newStatus } });',
        '            loadUsersList();',
        '        }',
        ...
        '            if (!drug_name) return;',
        '            await apiFetch("/api/master-drugs", { method: "POST", body: { zone: activeZone, drug_name } });',
        '            input.value = "";',
        '            await refreshData();',
        '        }',

        '        function renderMasterDrugs(filter = "") {',
        '            const tbody = document.getElementById("master-drugs-table");',
        '            tbody.innerHTML = "";',
        '            const f = filter.toLowerCase();',
        '            masterDrugsList.forEach(d => {',
        '                if (f && !d.toLowerCase().includes(f)) return;',
        '                const tr = document.createElement("tr");',
        '                tr.innerHTML = `',
        '                    <td><b>${d}</b></td>',
        '                    <td style="text-align:right;">',
        '                        <button class="action-link" onclick="selectDrugForDispensing(\'${d}\')">Dispense</button>',
        '                        <button class="action-link" onclick="renameDrug(\'${d}\')">Edit</button>',
        '                        <button class="action-link" style="color:var(--danger);" onclick="deleteDrug(\'${d}\')">Del</button>',
        '                    </td>',
        '                `;',
        '                tbody.appendChild(tr);',
        '            });',
        ...
        '        function filterMasterDrugs() {',
        '            const q = document.getElementById("master-search").value;',
        '            renderMasterDrugs(q);',
        '        }',

        '        function selectDrugForDispensing(dName) {',
        '            selectedDrugForDispense = dName;',
        '            document.getElementById("selected-drug-display").textContent = dName;',
        '            document.getElementById("dispense-qty").focus();',
        '        }',

        '        function addQty(n) {',
        '            const el = document.getElementById("dispense-qty");',
        '            el.value = Math.max(1, (parseInt(el.value) || 0) + n);',
        '        }',

        '        async function submitDispense() {',
        '            if (!selectedDrugForDispense) { alert("Please select a drug from Master Directory first."); return; }',
        ...
        '                method: "POST",',
        '                body: { zone: activeZone, drug_name: selectedDrugForDispense, qty }',
        '            });',
        ...
        '            const newName = prompt("Rename drug name:", oldName);',
        '            if (!newName || newName.trim() === oldName) return;',
        '            await apiFetch("/api/master-drugs/rename", { method: "PUT", body: { zone: activeZone, oldName, newName: newName.trim() } });',
        '            await refreshData();',
        '        }',

        '        async function deleteDrug(drug_name) {',
        '            if (!confirm("Remove " + drug_name + " from Master Directory?")) return;',
        '            await apiFetch("/api/master-drugs", { method: "DELETE", body: { zone: activeZone, drug_name } });',
        '            await refreshData();',
        '        }',
        ...
        '            const totals = {};',
        '            masterDrugsList.forEach(d => totals[d] = 0);',
        '            dispenseLogs.forEach(l => {',
        '                const dName = l.drug_name;',
        '                totals[dName] = (totals[dName] || 0) + l.qty;',
        '            });',

        '            const cumBody = document.getElementById("cumulative-table");',
        '            cumBody.innerHTML = "";',
        '            Object.keys(totals).sort().forEach(d => {',
        '                const tr = document.createElement("tr");',
        '                tr.innerHTML = `',
        '                    <td><b>${d}</b></td>',
        '                    <td style="text-align:right;"><span class="badge">${totals[d]}</span></td>',
        '                    <td style="text-align:right;">',
        '                        <button class="action-link" onclick="adjustCumulativeTotal(\'${d}\', ${totals[d]})">Set Total</button>',
        '                    </td>',
        '                `;',
        '                cumBody.appendChild(tr);',
        '            });',

        '            const txBody = document.getElementById("transactions-table");',
        '            txBody.innerHTML = "";',
        '            dispenseLogs.slice(0, 50).forEach(l => {',
        '                const timeStr = l.timestamp ? l.timestamp.split(" ")[1] || l.timestamp : "";',
        '                txBody.innerHTML += `<tr><td>${timeStr}</td><td>${l.drug_name}</td><td><b>${l.qty}</b></td><td>${l.entered_by}</td></tr>`;',
        '            });',
        '        }',

        '        async function adjustCumulativeTotal(drug_name, currentQty) {',
        '            const input = prompt("Set exact cumulative total quantity for " + drug_name + ":", currentQty);',
        '            if (input === null) return;',
        '            const new_qty = parseInt(input);',
        '            if (isNaN(new_qty) || new_qty < 0) { alert("Invalid number"); return; }',

        '            await apiFetch("/api/dispense/adjust-cumulative", {',
        '                method: "PUT",',
        '                body: { zone: activeZone, drug_name, new_qty }',
        '            });',
        '            await refreshData();',
        '        }',
        ...
        '            if (!zone) { alert("Select a target zone first."); return; }',
        '            if (!fileInput.files.length) { alert("Select a file (.xlsx, .xls, .json)."); return; }',
        ...
        '                        drugsArr = JSON.parse(e.target.result);',
        '                    } else {',
        '                        const data = new Uint8Array(e.target.result);',
        '                        const workbook = XLSX.read(data, { type: "array" });',
        '                        const firstSheet = workbook.SheetNames[0];',
        '                        const worksheet = workbook.Sheets[firstSheet];',
        '                        drugsArr = XLSX.utils.sheet_to_json(worksheet, { header: 1 });',
        '                    }',

        '                    const res = await apiFetch("/api/master-drugs/import", {',
        '                        method: "POST",',
        '                        body: { zone, mode, drugs: drugsArr }',
        '                    });',
        '                    alert(res.message || "Import completed successfully.");',
        '                    fileInput.value = "";',
        '                    await refreshData();',
        '                } catch (err) {',
        '                    alert("Failed to parse import file format.");',
        '                }',
        '            };',
        ...
        '            const totals = {};',
        '            masterDrugsList.forEach(d => totals[d] = 0);',
        '            dispenseLogs.forEach(l => totals[l.drug_name] = (totals[l.drug_name] || 0) + l.qty);',

        '            const data = Object.keys(totals).map(d => ({ "Drug Name": d, "Total Dispensed": totals[d], "Zone": activeZone }));',
        '            const worksheet = XLSX.utils.json_to_sheet(data);',
        '            const workbook = XLSX.utils.book_new();',
        '            XLSX.utils.book_append_sheet(workbook, worksheet, "Dispense Totals");',
        '            XLSX.writeFile(workbook, `PharmaSync_${activeZone}_Report.xlsx`);',
        '        }',
        ...
        '            doc.setFontSize(18);',
        '            doc.text("PHARMA-SYNC PRO | Dispense Report", 14, 20);',
        '            doc.setFontSize(11);',
        ...
        '            const rows = Object.keys(totals).map(d => [d, totals[d], activeZone]);',

        '            doc.autoTable({',
        '                startY: 35,',
        '                head: [["Drug Name", "Total Quantity", "Zone"]],',
        '                body: rows',
        '            });',

        '            doc.save(`PharmaSync_${activeZone}_Report.pdf`);',
        '        }',

        '        async function clearZoneTotals() {',
        '            if (!confirm("Are you sure you want to clear all dispense totals for zone " + activeZone + "?")) return;',
        '            await apiFetch("/api/dispense/clear/all", { method: "DELETE", body: { zone: activeZone } });',
        '            await refreshData();',
        '        }',
        ...
        '                            initAppSession({ role: currentUserRole, username: currentUsername });',
        '                        } catch(e) {',
        '                            handleLogout();',
        '                        }',
        '                    }',
...
app.listen(PORT, () => {
    console.log(`================================================================`);
    console.log(` PHARMA-SYNC PRO RUNNING ON PORT ${PORT}`);
    console.log(`================================================================`);
});
